...
public class AccountsResource extends HBankDataAccess{
	...
	public Response getAccountInternal(Long accountNumber) {
		...
		else
		{
			if(db2Account == null && id_safe == 999999999L)
			{
				...
				//* There exists a possibility that the last account in the named counter has been deleted. In which case we try once the old fashioned way
			}
			if(db2Account == null)
			{
	...
	public Response transferLocalExternal(@PathParam("id") String accountNumber,
		...
		Response myResponse = transferLocalInternal(accountNumber,transferLocal); 
		HBankDataAccess myHBankDataAccess = new HBankDataAccess();
		myHBankDataAccess.terminate();
	...
	public Response transferLocalInternal(@PathParam("id") String accountNumber,
		...
		if(new Long(accountNumber).longValue() == new Long(transferLocal.getTargetAccount().longValue()))
		{
			JSONObject error = new JSONObject();
	...
	public Response deleteAccountInternal(Long accountNumber) {
		...
		db2Account = db2Account.deleteAccount(accountNumber.longValue(), sortCode.intValue());
		if(db2Account != null)
		{
			response.put("sortCode", db2Account.getSortcode().trim());
			response.put("id",db2Account.getAccount_number());
